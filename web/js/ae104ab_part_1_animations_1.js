...
;(function($) {'use strict';
	$.fn.extend({
		LoaderAnimation : function(customOptions) {
			var defaults = {
				lineWidth : 15, /* set preloader's line width */
				color : "#ffffff", /* set preloader color */
				glowColor : "#005580", /* set shadow color */
				radius : 75, /* set the preloader radius (JUST FOR CIRCULAR PRELOADER) */
				font : "italic 13px Trebuchet MS" /* set preloader font (you can embed a font by css and use it here) */
			}, $container = $(this),
			// merging the custom options with the default ones
			options = $.extend(defaults, customOptions), self = this;
			 ...
			 *
			 */
			var lineWidth = options.lineWidth, color = options.color, glowColor = options.glowColor, radius = options.radius, font = options.font;
			 ...
			 *
			 */
			var $window = $(window), PI = Math.PI, startAngle = 1.5 * PI, endAngle = 0, supportsCanvas = !!document.createElement('canvas').getContext, canvasWidth = $(window).width(), canvasHeight = $(window).height(), $canvas, $fallbackHtml, ctx;
			...
			var draw = function() {
				var alphaPercentage = (2 / 100) * self.currentPercentage, positionX = canvasWidth / 2, positionY = canvasHeight / 2;
				...
				ctx.fillText("ONEOWEB " + (self.currentPercentage | 0) + "%", positionX, positionY);
